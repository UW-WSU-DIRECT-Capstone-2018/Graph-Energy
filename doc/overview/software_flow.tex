...
% -------------------------------------------------
% Place the nodes
\node [proc] (p1) {Parse data and input file};
\node [test, join] (t1) {Train?};
\node [proc] (p2) {Load trained net};
\node [proc, join] (p3) {Predict energy dist.};
\node [test, join] (t2) {Compare params?};
\node [proc] {Write output to file};
\node [proc, join] {Plot distribution(s)};
\node [term, join] {Termination};

% Other side nodes
\node [inpt, right=of p1] (i1) {Data files};
\node [inpt] (i2) {Input file}; 
\node [proc, right=of p2] {Generate net architecture};
\node [proc, join] {Train net};

% Simple left/right connections
\path (i1.west) to node [near start, xshift=1em] {} (p1);
  \draw [->,lcnorm] (i1.west) -- (p1);
